...
\item {\bf Biobase:} Obtaining a version of \verb+Biobase+ that
will work poses a slight conundrum. \verb+matchprobes+ depends on
\verb+Biobase+ 1.3.29, which itself depends on a developmental
...
We recommend that all of the metadata packages be installed so
...
interface, we recommend setting the permissions of these directories
...
done through the Site.pm file. The most important configuration
...
top of all shell scripts. It can be very useful for setting
environment variables such as PATH. (PATH must include the directories
containing standard shell tools, Ghostscript, and Netpbm binaries.)
